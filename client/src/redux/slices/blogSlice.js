  ...
  async ({ page = 1, limit = 6 }, thunkAPI) => {
    try {
      const response = await axios.get(
        `https://thuansever-production.up.railway.app/api/blogs?page=${page}&limit=${limit}`
      );
      return {
...
export const fetchBlogById = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await axios.get(
        `https://thuansever-production.up.railway.app/api/blog/${id}`
      );
      return {
...
export const fetchAllBlogFeatured = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const response = await axios.get(
        `https://thuansever-production.up.railway.app/api/blogs/featured`
      );
      return {
        blogs: response.data.data,
      };
